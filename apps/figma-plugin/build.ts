/**
 * Bundles the plugin into `dist/`.
 *
 * Two outputs, both fully self-contained:
 *   dist/code.js  — the sandbox thread
 *   dist/ui.html  — one HTML file with CSS and JS inlined
 *
 * Figma loads `ui.html` from disk, so nothing may reference an external URL.
 * The build asserts that afterwards rather than trusting the source.
 */
...
async function buildOnce(): Promise<void> {
  ...
  process.stdout.write('figma-plugin: built dist/code.js and dist/ui.html\n');
}
...
if (watch) {
  ...
  process.stdout.write('figma-plugin: watching src/…\n');
  watchFiles(SRC, { recursive: true }, () => {
    buildOnce().catch((error: unknown) => {
      process.stderr.write(`figma-plugin: ${(error as Error).message}\n`);
    });
  });
}
